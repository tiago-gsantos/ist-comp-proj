...
namespace udf {
  ...
  public:
    type_checker(std::shared_ptr<cdk::compiler> compiler, cdk::symbol_table<udf::symbol> &symtab, 
      std::shared_ptr<udf::symbol> func ,basic_ast_visitor *parent) :
  ...
  protected:
    bool pointerTypeComparison(std::shared_ptr<cdk::basic_type> left, std::shared_ptr<cdk::basic_type> right);
    ...
    void processPIDTExpression(cdk::binary_operation_node *const node, int lvl, bool acceptPointerPointer);
